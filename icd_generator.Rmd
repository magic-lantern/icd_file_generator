...
files <- tibble(
  ...
  file_name = c("data_files/CMS32_DESC_LONG_DX.txt",
                "data_files/CMS32_DESC_LONG_SG.txt",
                "data_files/icd10cm_codes_2017.txt",
                "data_files/icd10pcs_codes_2017.txt")
  )
...
search_for_code = function(v = NULL, search_term, ct = NULL) {
  dt <- icd_data
  ...
  apply(dt, 1, function(r) {
    stri_detect_fixed(r["code"], search_term)
  }) %>% subset(dt, .)
}

# example with all positional arguments
search_for_code(9, "334", "pc")

# just code to search for
search_for_code(search_term = 334)

# code and type (looking at both 9 & 10)
search_for_code(search_term = 334, ct = "pc")
...
generate_sample <- function(v, n_rows, d_cols, p_cols, g_cols, pct_empty = 0.20) {
  ...
  # runs in about 80 - 90 seconds
  # where as this line takes about 4 seconds for 1,000,000 rows
  all_data <- as.data.table(cbind(cbind(d, p), g))
  toc()
...
# for this example, feather is faster than fst
tic("feather timing")
